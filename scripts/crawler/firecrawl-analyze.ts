/**
 * Firecrawl Phase 2: 캡처된 markdown을 Gemini 2.5 Pro로 분석
 *
 * Phase 1 (firecrawl-capture-test.ts) 완료 후 실행
 * snapshots/{date}/{hospital}/ 폴더의 markdown을 읽어서 분석
 *
 * 실행: npx tsx scripts/crawler/firecrawl-analyze.ts
 * 단일: npx tsx scripts/crawler/firecrawl-analyze.ts --name "815의원"
 */
...
async function findLatestSnapshotDir(): Promise<string> {
  const entries = await fs.readdir(SNAPSHOTS_DIR);
  const dates = entries.filter((e) => /^\d{4}-\d{2}-\d{2}$/.test(e)).sort().reverse();
  if (dates.length === 0) {
    throw new Error('snapshots/ 폴더에 캡처 데이터가 없습니다. Phase 1을 먼저 실행하세요.');
  }
...
${combinedMarkdown}`;
  ...
  console.log(`  Gemini tokens: in=${tokens?.promptTokenCount ?? 0}, out=${tokens?.candidatesTokenCount ?? 0}`);
...
async function analyzeHospital(hospitalDir: string, hospitalName: string): Promise<AnalysisResult> {
  ...
  console.log(`\n${'═'.repeat(60)}`);
  console.log(`=== ${hospitalName} Gemini 분석 ===`);

  try {
    const { combined, pageCount } = await readMarkdowns(hospitalDir);
    console.log(`  Markdown: ${combined.length}자, ${pageCount}페이지`);
    ...
    const result = await analyzeWithGemini(hospitalName, combined);
    if (!result) {
      return {
        hospitalName, success: false, equipments: [], treatments: [], doctors: [],
        prices: 0, events: [], specialties: [], hospitalInfo: {},
    ...
    const outputPath = path.join(hospitalDir, 'gemini-analysis.json');
...
async function main(): Promise<void> {
  ...
  console.log('Firecrawl Phase 2: Gemini 분석');
  console.log('═'.repeat(60));

  const dateDir = await findLatestSnapshotDir();
  ...
  // 종합 보고
  console.log(`\n${'═'.repeat(60)}`);
  console.log('=== Phase 2 Gemini 분석 종합 결과 ===');
  console.log('═'.repeat(60));

  console.log('\n| 병원 | 장비 | 시술 | 가격 | 의사 | Markdown | 소요시간 |');
  console.log('|---|---|---|---|---|---|---|');

  for (const r of results) {
    if (r.success) {
      console.log(
        `| ${r.hospitalName} | ${r.equipments.length} | ${r.treatments.length} | ${r.prices} | ${r.doctors.length} | ${r.markdownLength.toLocaleString()}자 | ${r.elapsed.toFixed(1)}초 |`
      );
    } else {
  ...
  // Save summary
  const summaryPath = path.join(dateDir, 'analysis-summary.json');
  await fs.writeFile(summaryPath, JSON.stringify({
    ...
    totals: { equipments: totalEq, treatments: totalTr, prices: totalPr, doctors: totalDr },
  }, null, 2));

  console.log(`\n분석 결과가 각 병원 폴더에 gemini-analysis.json으로 저장되었습니다.`);
  console.log(`종합 요약: ${summaryPath}`);
}
