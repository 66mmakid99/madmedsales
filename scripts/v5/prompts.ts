...
## ★★★ 환각 방지 — 가장 중요한 규칙 ★★★
1. **원본에 명시적으로 언급된 장비/시술만 추출하세요.**
   "피부과이므로 보톡스가 있을 것이다" 같은 추론은 절대 금지합니다.
2. **확인할 수 없으면 추출하지 마세요.**
   이미지에서 글자가 불명확하면 추출하지 않습니다.
3. **일반적 시술 목록을 생성하지 마세요.**
   잘못된 예: 피부과 → 보톡스, 필러, 레이저토닝, 피코레이저...
   올바른 예: 페이지에 "피코토닝 5만원"이 있으면 → 피코토닝만 추출

## 중복 방지 규칙
1. 동일 시술/장비를 2번 이상 추출하지 마세요.
2. 공백·특수문자 차이만 있는 동일 시술은 1개만 추출:
   "써마지 FLX" = "써마지FLX" → 1개만
3. "바디 ○○" vs "페이스 ○○"처럼 부위만 다르면: 가격이 다르면 별도 항목, 같으면 1개만.

## 가격·이름 규칙
- "만원" → ×10000. "100만원" → 1000000 (100이 아님). 1,000원 미만이면 단위 오류 의심.
- 장비/시술 이름은 최소 2자. 1글자(빛, 실, 물)는 파싱 오류이므로 전체 이름을 확인하세요.
...
**★★★ 중요: 텍스트 맨 끝에 "아래는 비급여항목 가격표입니다"로 시작하는 별도 섹션이 있으면,
그 섹션의 모든 테이블 행을 treatments에 추가하고 반드시 source: "nongeubyeo"로 태깅하세요.
이 섹션은 전처리로 추출된 비급여 공식 가격표입니다. 한 행도 빠뜨리지 마세요. ★★★**
...
medical_devices에는 의료기기/레이저/에너지디바이스/주입기기만 포함하세요.
아래는 장비가 아닙니다. medical_devices에 절대 넣지 마세요:

[주사제/필러] 보톡스, 디스포트, 나보타, 쥬비덤, 레스틸렌, 볼루마, 벨로테로, 리쥬란, 쥬베룩, 프로파일로, 엘란쎄, 스컬트라, 래디에스, 수베란, 셀디엠, 스킨비브, Rejuran, Juvelook
[화장품/스킨케어] 세럼, 크림, 앰플, 마스크팩, 필링제, 선크림, 토너, 에센스
[시술 프로그램명] "프리미엄 리프팅 패키지", "안티에이징 프로그램", "VIP 관리" 등 패키지/프로그램명
[약품/성분] 성장인자, PRP, 줄기세포, 엑소좀, 비타민, 글루타치온, 태반주사, PDRN, PN, NCTF
[기타 비장비] 실(PDO, PCL, 녹는실), 봉합사, 보형물, 임플란트(치과), 교정기

위 항목은 treatments에 시술로 기록하세요 (예: "쥬비덤 필러" → treatments에 추가).
device_type: "injectable"인 것(booster, filler, botox, collagen_stimulator, lipolytic, thread)은
medical_devices가 아닌 treatments로 분류하세요.

단, 주입기 장비(더마샤인, 메조건, 큐어젯 등 기계 자체)는 medical_devices에 포함합니다.

## 미등록 장비/시술 처리 (필수)

[장비 사전]에 없는 장비(기계/디바이스만)를 발견하면:
...
1. **원본에 명시적으로 언급된 장비/시술만 추출하세요.**
   - 웹페이지 텍스트, 이미지 텍스트, 메뉴 등에 이름이 직접 나타나야 합니다.
   - "피부과이므로 보톡스가 있을 것이다" 같은 추론은 절대 금지합니다.

2. **확인할 수 없으면 추출하지 마세요.**
   - 이미지에서 글자가 불명확하면 추출하지 않습니다.
   - 부분적으로 보이는 이름은 추출하지 않습니다.
...
4. **confidence 필드 활용:**
   - 텍스트에서 명확히 확인 → "confirmed"
   - 이미지에서 확인했으나 일부 불명확 → "uncertain"
   - 추론 기반 → 추출하지 말 것 (confidence "low"도 사용 금지)

## 중복 방지 규칙

1. 동일 시술/장비를 2번 이상 추출하지 마세요.
2. "바디 ○○", "페이스 ○○"처럼 부위 접두어만 다른 경우:
   - 가격이 다르면 → 별도 항목으로 추출 (부위 명시)
   - 가격이 같으면 → 1개만 추출 (대표 이름 사용)
3. 공백·특수문자 차이만 있는 동일 시술은 1개만 추출:
   - "써마지 FLX" = "써마지FLX" → 1개만
   - "쥬베룩 (흉터개선)" = "쥬베룩" → 1개만 (설명은 description 필드에)

## 의사 경력/학력/학술활동 분류 규칙 (엄격)

1. **education**: 대학 졸업, 인턴, 레지던트, 전공의, 전문의 취득, 석사/박사
2. **career**: (전)/(현) 병원 원장/과장, 교수, 군의관, 자문의사
3. **academic_activities**: 아래와 같이 세분화하여 추출
   - 학회정회원: "~학회 정회원/종신회원" → academic_activities
   - 학회임원: "~학회 회장/이사/위원장" → academic_activities
   - 논문: SCI/SCIE/KCI 논문, journal of → academic_activities
   - 수상: 학술상/최우수상 수상 → academic_activities
   - 교과서집필: 교과서 집필/저자 → academic_activities
   - 편집위원: 편집위원, editorial board → academic_activities
   - 강연: 초청강연/연자/좌장 → academic_activities
   - 임상연구: 임상연구/clinical trial → academic_activities

**※ 분류 주의사항:**
- "대한~학회 정회원"은 career가 아님 → academic_activities
- "교과서 집필"은 career가 아님 → academic_activities
- 인턴/레지던트는 career가 아님 → education

4. **각 의사의 정보는 해당 의사의 프로필/소개 페이지에서 개별적으로 추출하세요.**
   - 병원 대표 페이지의 공통 소개가 아닌, 개별 의사 프로필을 확인하세요.
5. **학회 정회원/종신회원은 해당 의사 프로필에 명시된 경우만 추출하세요.**
   - 잘못된 예: 병원 소개에 "대한성형외과학회 소속"이라고 되어 있으면 모든 의사에 복사
   - 올바른 예: 각 의사 프로필에서 개별 확인
6. **의사 20명 이상인 대형 병원:**
   - 프로필 페이지가 없는 의사는 이름/직책만 추출하고 career를 빈 배열로 두세요.
   - 병원 공통 소개의 학회 목록을 모든 의사에 복사하지 마세요.
...
6. **가격이 1,000원 미만이면 단위 변환 오류를 의심하세요.** "100만원" → 1000000 (100이 아님).
7. **장비/시술 이름은 최소 2자 이상이어야 합니다.** 1글자 이름(빛, 실, 물 등)은 잘못된 파싱이므로 전체 이름을 확인하세요.
...
## ★★★ 환각 방지 ★★★
- 이미지에서 실제로 보이는 장비/시술만 추출하세요.
- 이미지에서 글자가 불명확하면 추출하지 마세요.
- "피부과이므로 보톡스가 있을 것이다" 같은 추론 금지. 보이는 것만 추출.
- 동일 시술/장비를 2번 이상 추출하지 마세요.
- 장비/시술 이름은 최소 2자. 1글자는 파싱 오류.
...
export function buildValidationPrompt(
  ...
  const weightDesc = isBlog
    ? '장비 15%, 시술 45%, 의사 25%, 학술활동 15% — 블로그 기준'
    : '장비 30%, 시술 40%, 의사 30%';
  const siteNote = isBlog
    ? `\n★ 이 병원은 블로그(${siteType})를 홈페이지로 사용합니다. 블로그 특성상 장비 목록과 가격 정보가 적을 수 있습니다. 시술 후기와 학술활동(의사 경력, 학회 참석 등) 중심으로 평가하세요.`
    : '';

  return `당신은 한국 피부과/성형외과 데이터 품질 검증 전문가입니다.${siteNote}
...
- overall: 세 항목의 가중 평균 (${weightDesc})
