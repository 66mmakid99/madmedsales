 ...
 *
 * 실행: npx tsx scripts/test-scoring.ts
 */
...
const invA = scoreInvestment(equipA, hospitalA);
const portA = scorePortfolio(equipA, treatA);
const scaleA = scoreScaleTrust(treatA, 5);
const marketingA = 60;
const profileScoreA = Math.round(invA * 0.35 + portA * 0.25 + scaleA * 0.25 + marketingA * 0.15);
const profileGradeA = assignProfileGrade(profileScoreA);
...
const matchGradeA = assignMatchGrade(matchA.totalScore);
...
const profileScoreB = Math.round(
  scoreInvestment(equipB, { opened_at: '2023-01-01' }) * 0.35 +
  scorePortfolio(equipB, treatB) * 0.25 +
  scoreScaleTrust(treatB, 1) * 0.25 +
  20 * 0.15
);
const profileGradeB = assignProfileGrade(profileScoreB);
...
const matchGradeB = assignMatchGrade(matchB.totalScore);
...
const matchGradeC = assignMatchGrade(matchC.totalScore);
...
const matchGradeD = assignMatchGrade(matchD.totalScore);
